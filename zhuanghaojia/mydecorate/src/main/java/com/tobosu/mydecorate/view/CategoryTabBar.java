...
//		defaultTabLayoutParams.s

		// 绘制高亮区域作为滑动分页指示器
		indicator = getResources().getDrawable(R.drawable.shape_bg_category_indicator);
		// 左右边界阴影效果
		...
		left_edge.setAlpha(0);
		right_edge.setAlpha(0);
	}

	// 绑定与CategoryTabStrip控件对应的ViewPager控件，实现联动
	public void setViewPager(ViewPager pager, ArrayList<String> titleList) {
		this.pager = pager;
...
//		notifyDataSetChanged();
	...
	private void calculateIndicatorRect(Rect rect) {
		...
		if(currentTab!=null){
			...
			rect.set(((int) left) + getPaddingLeft()+10, getPaddingTop() + currentTab.getTop() +category_text.getHeight()+5,
					((int) width) + getPaddingLeft()-10, currentTab.getTop() + getPaddingTop() + category_text.getTop() + category_text.getHeight());
