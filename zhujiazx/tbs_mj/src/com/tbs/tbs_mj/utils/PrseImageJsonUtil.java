		...
		try {
			...
			if (jsonObject.getInt("error_code") == 0) {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					map.put("img2", img2);
					if ("ImageActivity".equals(flag)) {
						// 逛图库
						JSONObject comInfo = jsonArray.getJSONObject(i).getJSONObject("com_info");
						map.put("hav_fav", jsonArray.getJSONObject(i).getString("hav_fav"));
						map.put("certification",comInfo.getString("certification"));
						map.put("comid", comInfo.getString("comid"));
						map.put("comsimpname", comInfo.getString("comsimpname"));
						map.put("jjb_logo", comInfo.getString("jjb_logo"));
						map.put("logosmall", comInfo.getString("logosmall"));

					} else if("DesignChartAcitivity".equals(flag)){
						// 设计图册
