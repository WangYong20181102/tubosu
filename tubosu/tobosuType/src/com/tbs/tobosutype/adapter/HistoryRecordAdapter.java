...
public class HistoryRecordAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            //数量
            ((MyViewHolder) holder).tvNum.setText(recordBeanList.get(position).getNum());
            //价格
            ((MyViewHolder) holder).tvPrice.setText(recordBeanList.get(position).getValue());
            //日期
            ((MyViewHolder) holder).tvDataTime.setText(recordBeanList.get(position).getData());
        }
    }
